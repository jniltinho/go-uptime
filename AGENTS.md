...
- Tags: plain SemVer, `vX.Y.Z`, starting at `v6.0.0`. The `v5.36.0-fork.N` tags are the history from when the project followed the upstream version; never create new ones. The Go module is `github.com/jniltinho/go-uptime/v7`: the `/v7` suffix follows the major version of the tags, so a future `v8.0.0` changes the module path (go.mod, the imports and the `-ldflags -X` of the `Makefile` and of the `Dockerfile`) in the same PR.

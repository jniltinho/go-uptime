...
![Go Uptime Grafana dashboard](../../.github/assets/grafana-dashboard.png)
...
### Success rate
```
sum(rate(go_uptime_results_total{success="true"}[30s])) by (key) / sum(rate(go_uptime_results_total[30s])) by (key)
```

### Response time
```
go_uptime_results_duration_seconds
```

### Total results per minute
```
sum(rate(go_uptime_results_total[5m])*60) by (key)
```

### Total successful results per minute
```
sum(rate(go_uptime_results_total{success="true"}[5m])*60) by (key)
```

### Total unsuccessful results per minute
```
sum(rate(go_uptime_results_total{success="false"}[5m])*60) by (key)
```

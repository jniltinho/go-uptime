...
|:--------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------|:----------------------------------------------------|
| `ui`                      | UI configuration                                                                                                                         | `{}`                                                |
| `ui.title`                | [Title of the document](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/title).                                                | `Health Dashboard ǀ Status`                         |
| `ui.description`          | Meta description for the page.                                                                                                           | `Automated status page...`.                         |
...
| `ui.custom-css`           | Custom CSS. Loaded before the stylesheet of the application, so rules that override it (for instance the thin scrollbar of the interface, or `body { font-variant-numeric: normal !important }` to turn off tabular figures) need `!important`. Changing the font family does not: `body { font-family: "Your font", sans-serif }` is enough. It is loaded before the stylesheet of the application: to override a variable of a theme (`:root`, `:root.dark`, `:root.theme-bio`), use `!important` or a more specific selector. | `""`                                                |
| `ui.dark-mode`            | Whether the dark theme is the default one, when `ui.default-theme` is not set. The theme chosen with the theme selector is remembered in the browser and takes precedence. | `true` |
| `ui.default-theme`        | The theme used when the visitor has not chosen one: `dark`, `light` or `bio` (a light theme in teal, blue and navy, with a navy header). When it is set it wins over `ui.dark-mode`, and a value that is not one of the three makes the configuration invalid. | `""` (see `ui.dark-mode`) |

...
# Support
If you need help with this integration, please create an issue at https://github.com/jniltinho/go-uptime/issues
...
2. There are two ways to add an integration to a service:
   ...
   * **If you are creating a new service for your integration**: Please read our documentation in section [Configuring Services and Integrations](https://support.pagerduty.com/docs/services-and-integrations#section-configuring-services-and-integrations) and follow the steps outlined in the [Create a New Service](https://support.pagerduty.com/docs/services-and-integrations#section-create-a-new-service) section, selecting **Go Uptime** as the **Integration Type** in step 4. Continue with the In Go Uptime section (below) once you have finished these steps.
...
- **If all conditions are not met 3 (`endpoints[].alerts[].failure-threshold`) times in a row**: Go Uptime will create a new incident
- **If, after an incident has been triggered, all conditions are met 5 (`endpoints[].alerts[].success-threshold`) times in a row _AND_ `endpoints[].alerts[].send-on-resolved` is set to `true`**: Go Uptime will resolve the triggered incident

It is highly recommended to set `endpoints[].alerts[].send-on-resolved` to true for alerts of type `pagerduty`.


# How to Uninstall
1. Navigate to the PagerDuty service you'd like to uninstall the Go Uptime integration from
2. Click on the **Integration** tab
3. Click on the **Go Uptime** integration
...
in your Go Uptime configuration files or simply remove the integration key from the configuration file.
